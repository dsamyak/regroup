...
import { generateSessionQuestions, type Question } from '../utils/questionBank';
import { sounds } from '../utils/audio';
import QuestionRenderer from '../components/quiz/QuestionRenderer';

const WORLDS = [
  { id: 0, name: 'Number Village', icon: '🏘️', color: '#ff4081', desc: 'Questions 1–10' },
  { id: 1, name: 'Carry Canyon', icon: '🏜️', color: '#4caf50', desc: 'Questions 11–20' },
  { id: 2, name: 'Borrow Bay', icon: '🌊', color: '#03a9f4', desc: 'Questions 21–30' },
  { id: 3, name: 'Place Value Plains', icon: '🌾', color: '#00bcd4', desc: 'Questions 31–40' },
  { id: 4, name: 'Regrouping Ridge', icon: '⛰️', color: '#ff5722', desc: 'Questions 41–50' },
  { id: 5, name: 'Column Castle', icon: '🏰', color: '#673ab7', desc: 'Questions 51–60' },
  { id: 6, name: 'Tens Tower', icon: '🗼', color: '#e91e63', desc: 'Questions 61–70' },
  { id: 7, name: 'Hundreds Heights', icon: '🏔️', color: '#9c27b0', desc: 'Questions 71–80' },
  { id: 8, name: 'Word Problem Woods', icon: '🌲', color: '#8bc34a', desc: 'Questions 81–90' },
  { id: 9, name: 'Treasure Vault', icon: '💎', color: '#ff9800', desc: 'Questions 91–100' },
];

function calcXP(attempt: number, streak: number) {
...
export default function PlayPhase({ onComplete }: Props) {
  ...
  const [worldResults, setWorldResults] = useState<Record<number, WorldResult>>({});
  ...
  const [feedback, setFeedback] = useState<{ type: string; message: string; sub: string } | null>(null);
  const [answered, setAnswered] = useState(false);
  const [xpPopup, setXpPopup] = useState<string | null>(null);
  const [worldComplete, setWorldComplete] = useState(false);
  const [worldQuestions, setWorldQuestions] = useState<Question[]>([]);

  const allQuestions = useMemo(() => generateSessionQuestions(), []);

  const q = worldQuestions[qIndex];

  const startWorld = useCallback((worldId: number) => {
    const filtered = allQuestions.filter(q => q.world === worldId);
          ...
          {WORLDS.map((w, i) => {
            ...
            return (
              <div key={w.id} className={`world-card ${unlocked ? 'unlocked' : 'locked'} ${completed ? 'completed' : ''}`}
                onClick={() => unlocked && startWorld(i)} style={{ '--world-color': w.color } as any}>
